...
# 3. Create TEST dataframe
 ...
 # 5. Extracts only the measurements on the mean and standard deviation for each measurement from joint dataframe
      ...
      dfms <- dfall[, c( "Activity", "Subject", grep( "[Mm]ean|[Ss]td", names(dfall), value = TRUE ) ) ]


      
# 6. Creates tidy data set with the average of each variable for each activity and each subject
      
      dfmelt <- melt(dfms, id = c("Subject", "Activity"), measure.vars = c( names(dfms)[ 3:dim(dfms)[2] ] ) ) #puts data in long format
      ...
      write.table(dftidy,"dftidy.txt", row.names = FALSE) # save output data on working directory
